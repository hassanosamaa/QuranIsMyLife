...
export default function Header() {
  ...
  return (
    <div className="relative overflow-hidden flex justify-between items-center pt-5 pr-2 
     pb-0 shadow-md mb-[18px] rounded-[20px] bg-gradient-to-r from-sky-950 to-sky-500 ">
      ...
      <div className="w-full flex flex-col justify-center items-center gap-5">
        ...
        <h3 className="text-gray-900 font-[700] text-[12px] md:text-[18px] ">{nameReciters[refresh]}</h3>
        <button className="absolute top-0 right-0 p-4" onClick={handleRefresh}>
          <Image width={25} height={25} src={fresh} alt="fresh"/>
        </button>
      </div>
    </div>
  );
